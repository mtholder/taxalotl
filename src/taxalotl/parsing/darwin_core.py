...
#  Ajuga pyramidalis L.
_lower = "a-záåäàãçëéèïíøöóü'×?"
_upper = "A-ZÄÁÅÁÁÇČÐĎĐÉÉÎİŁŘŠŚŞȘÔØÖÔÓÜÚŽ"
_epithet = " +[%s0-9.-]+" % _lower

# Matches a canonical name
_genus_re = "[A-ZÖ{x}-]+".format(x=_lower)
_epithets_re = "(|{e}|{e}{e}|{e}{e}{e})".format(e=_epithet)
_canon_re = "{}{}".format(_genus_re, _epithets_re)
_auth_re = (
    " +(d'|von |van |de |dem |der |da |del |di |le |f\\. |[{}(])(..|\\.).*".format(
        _upper
    )
)
_with_subg = "{g} \\({g}\\){e}".format(g=_genus_re, e=_epithets_re)
_trimmer = re.compile("({})({})".format(_canon_re, _auth_re))
_subg_trimmer = re.compile("({})({})".format(_with_subg, _auth_re))
_year_re = re.compile(".*, [12][0-9][0-9][0-9?]\\)?")
_has_digit = re.compile(".*[0-9].*")


def canonical_name(name):
    if " phage " in name or name.endswith(" phage"):
        return name
    if " virus " in name or name.endswith(" virus"):
        return name
    ...
    with io.open(source, "r", encoding="utf-8") as infile:
        with OutFile(destination) as outfile:
            for line in infile:
                ...
                outfile.write("\n")
                if i % 500000 == 0:
                    _LOG.info("{} {} => {}".format(i, scientific, canenc))
                i += 1
...
def read_gbif_projection(proj_filepath, itd, field_to_index, do_gbif_checks):
    ...
    with io.open(proj_filepath, "r", encoding="utf-8") as inp:
        for line_num, row in enumerate(inp):
            ...
            if (
                ("IRMNG Homonym" in source)
                or ("Interim Register of Marine" in source)
                or ("International Plant Names Index" in source)
                or (source == "d9a4eedb-e985-4456-ad46-3df8472e00e8")
            ):
            ...
            if tstatus == "synonym" or (
                tstatus == "doubtful" and taxon_id not in not_doubtful
            ):
                to_remove.add(taxon_id)
                continue
            if do_gbif_checks and (
                tstatus != "accepted" and taxon_id not in not_doubtful
            ):
            ...
            if count % 100000 == 0:
                _LOG.info(
                    "lines={} #syn={} #roots={}".format(
                        count, len(itd.synonyms), len(itd.root_nodes)
                    )
                )
    ...
    itd.details_log["removed_sources"] = [
        "Interim Register of Marine",
        "IRMNG Homonym",
