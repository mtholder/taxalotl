...
def download_csv_for_family(fam_dir, fam_html_fp, url_pref):
    ...
    if len(csva) != 1:
        raise RuntimeError(
            "Not just 1 CSV type links in {} : {}".format(fam_html_fp, csva)
        )
    csv_link = csva[0]
    csv_rel_url = csv_link["href"]
    template = (
        "{}{}" if (url_pref.endswith("/") or csv_rel_url.startswith("/")) else "{}/{}"
    )
...
def scrape_families_from_higher_group(out_dir, top_file):
    ...
    for list_item in nametree_list:
        ...
        if not os.path.exists(fam_dest):
            _LOG.debug(
                "Starting download from url = {} to {}".format(fam_url, fam_dest)
            )
            download_large_file(fam_url, fam_dest)
            _num_downloads_this_session += 1
            _LOG.debug("Download completed to .".format(fam_url, fam_dest))
...
def _gen_line(ls):
    try:
        ...
        m = 'Could not serialize to ASCII: "{}"   converted to "{}"'
        old = '", "'.join(ls)
        new = '", "'.join(conv)
...
def normalize_plantlist_file(inp_fp, out_dir, family, maj_group_id):
    _LOG.info("{} to {}".format(inp_fp, out_dir))
    ...
    with io.open(inp_fp, "r", encoding="utf-8") as csvfile:
        ...
        _LOG.info("header = {}".format(header))
        for n, raw_row in enumerate(csvreader):
            ...
            if fam != family:
                raise RuntimeError(
                    "Unexpected family in taxon {} of {}: {}".format(n, family, row)
                )
            genus = row[4]
            assert genus
