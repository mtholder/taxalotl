...
class TaxalotlConfig(object):
        ...
        if filepath is None:
            if os.path.exists("taxalotl.conf"):
                ...
                home_loc = os.path.expanduser(
                    "~/.opentreeoflife/taxalotl/taxalotl.conf"
                )
                if os.path.exists(home_loc):
                    filepath = home_loc
        if filepath is None:
            m = (
                "filepath to taxalotl.conf must be provided (or it must be in the current dir, "
    ...
    def get_fragment_from_part_name(self, parts_key, recurse=True):
        try:
            ...
            try:
                ...
                if not recurse:
                    raise
                from .commands import cache_separator_names
    ...
    def get_part_inp_taxdir(self, part_key, taxonomy_id):
        from .cmds.partitions import INP_TAXONOMY_DIRNAME
    ...
    def get_par_and_par_misc_taxdir(self, part_key, taxonomy_id):
        from .cmds.partitions import INP_TAXONOMY_DIRNAME, MISC_DIRNAME
        ...
        pmtd = os.path.join(
            self.partitioned_dir, misc_df, INP_TAXONOMY_DIRNAME, taxonomy_id
        )
    ...
    def get_resource_by_id(self, res_id):
        # from .resource_wrapper import ResourceWrapper type: (str) -> ResourceWrapper
        try:
